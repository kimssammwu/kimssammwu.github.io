  ...
  return (
    <div>
      ...
      <div className="flex justify-center">
            ...
            {markdownFiles.length > 0 ? (
              markdownFiles.map(({ node }) => {
                ...
                return (
                  <li key={node.id}>
                    <PostCard
                      post_title={node.frontmatter.title}
        ...
        {/* more than 1024 px */}
        <div className="w-[1200px] h-screen flex justify-evenly mt-8 hidden lg:flex">
          {/* 게시글 블록 */}
          <ul className="">
            {markdownFiles.length > 0 ? (
              markdownFiles.map(({ node }) => {
          ...
          {/* 추가 데이터 블록 */}
          <div className="w-72 h-full border-l px-2.5"></div>
        </div>
      </div>
    </div>
  );
...
export const Head: HeadFC = () => (
  <>
    ...
    <meta
      ...
      content="4-rsllo56teB73PBKZShvSFjQS76vIUBKnE06ZPjCTo"
    />
  </>
);
